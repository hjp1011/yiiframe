...
class Init implements BootstrapInterface
{
    ...
    public function afreshLoad($merchant_id)
    {
        ...
        try {
            ...
            // 获取 ip 配置
            $this->id != AppEnum::CONSOLE && $sys_ip_blacklist_open = Yii::$app->debris->addonConfig(true,'IpBlack')['sys_ip_blacklist_open']??'';
            // 初始化模块
        ...
        // ip黑名单拦截器
        if ($sys_ip_blacklist_open == true && AddonHelper::isInstall('IpBlack') && !in_array(Yii::$app->id, [AppEnum::CONSOLE])) {
