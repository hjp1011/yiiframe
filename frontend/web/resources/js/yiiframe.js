$(document).ready(function () {
    ...
    if (isDefaultShow === false) {
        var type = $('.rfTopMenu').eq(1).data('type');
        if (type) {
            $('.rfLeftMenu-' + type).removeClass('hide');
            $('.rfTopMenu').eq(1).addClass('rfTopMenuHover');
        }
    }
});
...
// 错误提示
function rfError(title, text,button) {
    if (!button) {
        button = '确认';
    }
...
// 警告提示
function rfWarning(title, text,button) {
    if (!button) {
        button = '确认';
    }
    let dialogText = rfText(text);
...
// 普通提示
function rfAffirm(title, text,button) {
    if (!button) {
        button = '确认';
    }
    let dialogText = rfText(text);
...
// 信息提示
function rfInfo(title, text,button) {
    if (!button) {
        button = '确认';
    }
    let dialogText = rfText(text);
...
// 成功提示
function rfSuccess(title, text,button) {
    if (!button) {
        button = '确认';
    }
    let dialogText = rfText(text);
